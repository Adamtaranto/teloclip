...
class TestWriteFastaSequences:
    ...
    @patch('teloclip.bio_io.SeqIO.write')
    def test_write_fasta_sequences_to_stdout(self, mock_seqio_write):
        ...
        mock_seqio_write.assert_called()
        # Check that one of the calls was made with sys.stdout
        calls = mock_seqio_write.call_args_list
        stdout_calls = [call for call in calls if call[0][1] == sys.stdout]
        assert len(stdout_calls) > 0, (
            'Expected at least one call to SeqIO.write with sys.stdout'
        )
...
class TestStreamWriteFastaSequences:
    ...
    @patch('teloclip.bio_io.SeqIO.write')
    def test_stream_write_fasta_sequences_to_stdout(self, mock_seqio_write):
        ...
        # Should have called SeqIO.write with stdout
        mock_seqio_write.assert_called()
        # Check that the call was made with sys.stdout
        args, kwargs = mock_seqio_write.call_args
        assert args[1] == sys.stdout, (
            'Expected SeqIO.write to be called with sys.stdout'
        )
...
class TestBioIoIntegration:
    ...
    def test_streaming_vs_loading_consistency(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.fna') as f:
            ...
            streamed = {
                seq_id: (desc, seq)
                for seq_id, desc, seq in stream_fasta_sequences(f.name)
            }
