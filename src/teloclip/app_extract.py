...
from teloclip.seqops import writefasta, writeClip, revComp, read_fai, filterList
...
def splitbycontig(alignments=None, contigs=None, prefix=None, outdir=None):
    ...
    for name in contigs.keys():
        ...
        if rightCount:
            # Open output file
            outfileR = os.path.join(
                outdir, "_".join([base, "R", str(rightCount)]) + ".fasta"
            )
            # Output reads aligned to right end of contig
            with open(outfileR, "w") as fileR:
                for aln in alignments[name][
                    "R"
                ]:  # i.e [(alnStart,alnEnd,rightClipLen,readSeq,readname)]
                    masked = aln[3][: -aln[2]] + aln[3][-aln[2] :].lower()
                    writefasta(fileR, str(aln[4]), masked)
        if leftCount:
            # Open output file
            outfileL = os.path.join(
                outdir, "_".join([base, "L", str(leftCount)]) + ".fasta"
            )
            # Output reads aligned to left end of contig
            with open(outfileL, "w") as fileL:
                for aln in alignments[name][
                    "L"
                ]:  # i.e #[(alnStart,alnEnd,leftClipLen,readSeq,readname)]
                    masked = aln[3][: aln[2]].lower() + aln[3][aln[2] :]
                    writefasta(fileL, str(aln[4]), masked)
...
def clipdown(args, ID=None, alnList=None, contig=None, temp=None, mode=None):
    ...
    if mode == "R":
        for aln in alnList:  # i.e [(alnStart,alnEnd,rightClipLen,readSeq,readname)]
            ...
            maplen = aln[1] - aln[0]
            writeClip(idx, pad, gap, overhang, maplen)
    elif mode == "L":
    ...
    if Q1:
        ...
        if mode == "R":
            log(
                "Selected alignment %s: %s"
                % (str(targetIdx + 1), alnList[targetIdx][3][-aln[2] :])
            )
        elif mode == "L":
        ...
        if "A" in mode:
            ...
            if Q2 == "S":
                ...
                if mode == "C":
                    log(
                        "There are %s remaining alignments. Aligned reads have already been error corrected. \n\
                    ...
                    log(
                        "There are %s remaining alignments. You may choose from the following options: \n\
